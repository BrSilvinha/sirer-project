...
import { 
    ...
    Spinner, Alert, Table, Dropdown, ButtonGroup, InputGroup,
...
const ProductosManagement = () => {
    ...
    const fetchProductos = useCallback(async () => {
        try {
            setLoading(true);
            const params = {
                categoria_id: filtros.categoria !== 'todas' ? filtros.categoria : undefined,
                disponible: filtros.disponibilidad !== 'todos' ? filtros.disponibilidad === 'disponibles' : undefined
            };
            ...
            if (filtros.busqueda) {
                productosData = productosData.filter(producto => 
                    ...
                    producto.categoria?.nombre.toLowerCase().includes(filtros.busqueda.toLowerCase())
                );
            }
            ...
            const productosPaginados = productosData.slice(startIndex, endIndex);
    ...
    }, [filtros, currentPage, categorias]);
